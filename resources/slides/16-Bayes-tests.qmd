...
Suppose we have univariate data $Y_i \overset{iid}{\sim} \N(\theta, 1)$, $\Y = (y_i, \ldots, y_n)^T$
...
- If $\theta \mid \tau_0, \cal{H}_1 \sim \N(0, 1/\tau_0)$,  then $\theta_0  \mid \cal{H}_1$ has a $\textsf{Cauchy}(0,1)$ distribution!  Recommended by Jeffreys (1961)
...
- **intrinsic prior** on $\theta$ given $\cal{H}_1$ in model $\Y \mid \theta \sim \N(\theta, \sigma^2)$
$$
