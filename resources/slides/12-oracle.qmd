...
\begin{align*}
& \argmax_{\b} -\frac{1}{2}  \| \Y - \X \bhat \|^2 - \frac{1}{2} \sum_j(\beta_j -\hat{\beta}_j)^2 + \sum_j \log(p(\beta_j \mid \lambda)) \\
...
- if $\lim_{|\beta_j| \to \infty} p^\prime_\lambda(|\beta|) = 0$
   then 
...
:::: {.columns}
::: {.column width="60%"}
As sufficient condition for a thresholding rule $\hat{\beta}_\lambda = 0$ is if 
$$0 < \min \left\{ |\beta_j| + p^\prime_\lambda(|\beta_j|)\right\}$$ 

- if $|\hat{\beta}_j| < \min \left\{ |\beta_j| + p^\prime_\lambda(|\beta_j|) \right\}$
...
- if $|\hat{\beta}_j| > \min \left\{ |\beta_j| + p^\prime_\lambda(|\beta_j|) \right\}$ multiple crossings (local roots)

- a sufficient and necessary condition for continuity is that the  minimum of $|\beta_j| + p^\prime_\lambda(|\beta_j|)$ is obtained at zero
...
$$\min \left\{ |\beta_j| + p^\prime_\lambda(|\beta_j|)\right\}  = (1 + \lambda)|\beta_j|$$
is zero  
...
$$\min \left\{ |\beta_j| + p^\prime_\lambda(|\beta_j|)\right\}  = (|\beta_j| +  \lambda) > 0 $$
...
\begin{align*}
 ...
 \tau_j \mid \lambda_j & \sim \Exp(\lambda_^2/2) \\
