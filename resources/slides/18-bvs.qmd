...
- what if the true model $\g_T$ is not in $\Gamma$?   What can we say?
    ...
    - $\M$-closed;  realize that $(p_\g) = 0 \ \forall \g \in \G$ and  is nonsense but know $\g_T$, however want to use models in $\G$ only 
    - $\M$-open; realize that $(p_\g) = 0 \ \forall \g \in \G$ and  is nonsense but know $\g_T$
    - latter is related to "stacking" which is a frequentist method of ensemble learning
      using cross-validation;
      see Clyde & Iversen (2013) for the curious to motivate via decision theory
