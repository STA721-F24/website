...
We need to show that $\bhat$ and $\bhat_\V$ are the same for all $\Y$. Since both $\P$ and  $\P_\V$ are projections onto $C(\X)$, $\bhat$ and $\bhat_\V$ will be the same iff  

1. Show that $\P = \P_V$ iff $C(\X) = C(\V^{-1} \X)$
2. Show that $C(\X) = C(\V^{-1} \X)$ iff $C(\X) = C(\V\X)$
3. Show that $C(\X) = C(\V\X)$ iff $\V$ can be written as 
...
::: {.Corollary}
Suppose $\V = \X \Psib \X ^ T + \Omegab$.  Then $\bhat_\V = \bhat_\Omegab$
:::
